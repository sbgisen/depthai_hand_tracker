...
class HandTrackerBpf:
    ...
    def __init__(self, input_src=None,
        ...
        if input_src is None or input_src == "rgb" or input_src == "rgb_laconic":
            ...
            print(
                f"Internal camera image size: {self.img_w} x {self.img_h} - crop_w:{self.crop_w} pad_h: {self.pad_h}")
        ...
        if self.input_type != "rgb":
            ...
            if self.crop_w:
                print("Cropping on width :", self.crop_w)
            self.crop_h = max((self.img_h - self.frame_size) // 2, 0)
            if self.crop_h:
                print("Cropping on height :", self.crop_h)

            self.pad_w = max((self.frame_size - self.img_w) // 2, 0)
            if self.pad_w:
                print("Padding on width :", self.pad_w)
            self.pad_h = max((self.frame_size - self.img_h) // 2, 0)
            if self.pad_h:
                print("Padding on height :", self.pad_h)
    ...
    def lm_postprocess(self, hand, inference):
        ...
        if hand.lm_score > self.lm_score_thresh:
            ...
            # hand.norm_landmarks contains the normalized ([0:1]) 3D coordinates of
            # landmarks in the square rotated body bounding box
    ...
    def spatial_loc_roi_from_palm_center(self, hand):
        ...
        rect_center = dai.Point2f(int(hand.pd_box[0] *
                                      self.frame_size)
                                  + half_size
                                  - zone_size
                                  // 2
                                  + self.crop_w, int(hand.pd_box[1]
                                                     * self.frame_size) +
                                  half_size
                                  - zone_size
                                  // 2
    ...
    def next_frame(self):
        ...
        if not self.use_previous_landmarks:
            ...
            if focus_zone:
                ...
                frame_nn.setData(
                    to_planar(
                        focus_frame if focus_zone else square_frame,
                        (self.pd_input_length,
                         self.pd_input_length)))
        ...
        if self.use_lm:
            ...
            for i, h in enumerate(self.hands):
                ...
                if i == 0:
                    lm_rtrip_time = now()  # We measure only for the first hand
            for i, h in enumerate(self.hands):
            ...
            if len(self.hands) == 2:
                dist_rect_centers = mpu.distance(
                    np.array(
                        (self.hands[0].rect_x_center_a, self.hands[0].rect_y_center_a)), np.array(
                        (self.hands[1].rect_x_center_a, self.hands[1].rect_y_center_a)))
                if dist_rect_centers < 5:
                    ...
                    if self.trace & 1:
                        print("!!! Removing one hand because too close to the other one")
            ...
            if self.solo:
                if nb_hands == 1:
                    # hand_from_landmarks will be used to initialize the bounding rotated
                    # rectangle (ROI) in the next frame
                    ...
                    if hand_zone_label is not None:
                        if nb_hands == 2:
                            ...
                            self.hands_from_landmarks = [
                                mpu.hand_landmarks_to_rect(
                                    self.hands[0]), mpu.hand_landmarks_to_rect(
                                    self.hands[1])]
                            self.use_previous_landmarks = True
                        elif nb_hands == 1:
            ...
            for hand in self.hands:
                # If we added padding to make the image square, we need to remove this
                # padding from landmark coordinates and from rect_points
                if self.pad_h > 0:
    ...
    def exit(self):
        ...
        if self.stats:
            ...
            if self.input_type != "rgb":
                print(
                    f"Body pose estimation round trip      : {self.glob_bpf_rtrip_time/self.nb_bpf_inferences*1000:.1f} ms")
                print(
                ...
                if self.use_lm and self.nb_lm_inferences:
                    print(
                        f"Hand landmark round trip             : {self.glob_lm_rtrip_time/self.nb_frames_lm_inference*1000:.1f} ms")
            if self.xyz:
                print(

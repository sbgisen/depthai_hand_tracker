...
def torso_visible(scores):
    ...
    return ((scores[BODY_KP["left_hip"]] > body_score_thresh
            or scores[BODY_KP["right_hip"]] > body_score_thresh)
            and (scores[BODY_KP["left_shoulder"]] > body_score_thresh or
...
def determine_crop_region(scores, x, y):
    ...
    if torso_visible(scores):
        ...
        max_torso_yrange, max_torso_xrange, max_body_yrange, max_body_xrange = determine_torso_and_body_range(
            x, y, scores, center_x, center_y)
        crop_length_half = max(
            max_torso_xrange * 1.9,
            max_torso_yrange * 1.9,
            max_body_yrange * 1.2,
            max_body_xrange * 1.2)
        crop_length_half = int(round(min(crop_length_half, max(
            center_x, img_w - center_x, center_y, img_h - center_y))))
        ...
        if crop_length_half > max(img_w, img_h) / 2:
            ...
            return {
                'xmin': crop_corner[0],
                'ymin': crop_corner[1],
                'xmax': crop_corner[0] + crop_length,
                'ymax': crop_corner[1] + crop_length,
                'size': crop_length}
    else:
        return init_crop_region
...
def estimate_focus_zone_size(scale=1.0):
    ...
    for s in segments:
        ...
        if body_scores[id0] > body_score_thresh and body_scores[id1] > body_score_thresh:
            ...
            lengths.append(l)
    if lengths:
        if (body_scores[BODY_KP["left_hip"]] < body_score_thresh
            and body_scores[BODY_KP["right_hip"]] < body_score_thresh
            or body_scores[BODY_KP["left_shoulder"]] < body_score_thresh
                and body_scores[BODY_KP["right_shoulder"]] < body_score_thresh):
            coef = 1.0
        else:
            coef = 1.0
...
while True:
        ...
        for i in range(2):
            ...
            if pd_score >= ${_pd_score_thresh} and box_size > 0:
                if zone:
                    ...
                    # We need box_x, box_y, box_size, kp0_x, kp0_y, kp2_x, kp2_y expressed in
                    # normalized coords in squared source image (sqn_)!
    ...
    for ih, hand in enumerate(detected_hands):
        ...
        if lm_score > ${_lm_score_thresh}:
            ...
            xyz_zone = [int(roi.topLeft().x - crop_w), int(roi.topLeft().y),
                        int(roi.bottomRight().x - crop_w), int(roi.bottomRight().y)]
            ${_IF_XYZ}
            ...
            y1 = 0.25 * (sqn_lms[2 * id_index_mcp + 1] + sqn_lms[2 * id_ring_mcp + 1]) + \
                0.5 * sqn_lms[2 * id_middle_mcp + 1]
    ...
    send_result_hands(
        2 if send_new_frame_to_branch == 1 else 0,
        nb_lm_inf,
        hand_landmarks["lm_score"],
        hand_landmarks["handedness"],
        hand_landmarks["rect_center_x"],
        hand_landmarks["rect_center_y"],
        hand_landmarks["rect_size"],
        hand_landmarks["rotation"],
        hand_landmarks["rrn_lms"],
        hand_landmarks["sqn_lms"],
        hand_landmarks["world_lms"],
        hand_landmarks["xyz"],
